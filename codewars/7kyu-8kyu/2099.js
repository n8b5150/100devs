...
function  calculateAge(yob, year) {
    let years = year - yob
    if (years == 1) return `You are ${years} year old.`
    if (years == -1) return `You will be born in ${Math.abs(years)} year.`
    if (years > 0) return `You are ${years} years old.`
    // You are 4 years old.
    if (years < 0) return `You will be born in ${Math.abs(years)} years.`
    // You will be born in 10 years.
    if (years == 0) return "You were born this very year!"
